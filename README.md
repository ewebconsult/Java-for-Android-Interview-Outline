...
-----------------------------------------------
super关键字的应用场景:
   1):在子类方法中,调用父类被覆盖掉的方法,如:super.fly();不能省略.
...
如:企鹅(Penguin)和鸵鸟是鸟类中的一个特殊品种，所以企鹅/鸵鸟类是鸟类的一个子类，但是鸟类有飞翔的功能，但是对应企鹅/鸵鸟，飞翔的行为显然不适合于它。

----------------------------------------------------------
当父类的某一个行为特征不适于子类的自身特征的时候?
解决方案:子类重新覆盖该方法,并重新编写方法体.------->方法的覆盖.

----------------------------------------------------------
方法重载(Overload):在同一个类中,方法名相同,参数列表不同. "两同一不同"
方法覆盖(Override):存在继承关系,强调的是父类和子类的方法.
    ...
    王道的做法:子类要覆盖父类方法的时候,直接把方法的定义拷贝过来,重新编写方法体即可.
...
继承关系: 在原有类的继承之上,加以拓展,从而产生新的子类的过程.
 继承关系,其实是一种"is A"关系,子类其实是父类的一种特殊情况.
 狗 继承于 动物,狗是一种特殊的动物.
 猫 继承于 动物,猫是一种特殊的动物.
...
--------------------------------------------------------
在Java中,即使A类没有显示的去继承B类,此时A类的父类是Object.
class Employee{} 等价于
class Employee extends Object{}
推论:任何类(除了Object),都有一个父类.

--------------------------------------------------------

在Java中类的继承,只允许单继承,不支持多继承.
   一个类,只能有一个直接的父类,但是允许多重继承.
